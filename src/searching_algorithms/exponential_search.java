package searching_algorithms;
import java.util.*;

public class exponential_search{

    // return position of / first occurrence of x in array
    static int exponenitalSearch(int arr[], int n, int x){
        // if x is present in first location
        if(arr[0] == x)
            return 0;

        //find range for binary search by
        //repeated doubling
        int i = 1;
        while(i<n && arr[i] <= x)
            i = i*2;
        //call binary search for the found range
        return Arrays.binarySearch(arr, i/2, Math.min(i, n-1), x);

    }

    //Driver code
    public static void main(String args[]){
        int arr[] = {2,3,4,10,40};
        int x =10;
        int result = exponenitalSearch(arr, arr.length, x);
        System.out.println((result < 0) ? "Element is not present in array" : "Element is present at index: " + result);
    }
}
