...
public class SellerDAO {
	...
	public Seller getByName(String name) throws SQLException {
		...
		if(resultset.next()) {
			...
			seller.setAddress(resultset.getString("address"));
			seller.setCompany(resultset.getString("company"));
			seller.setBusiness(resultset.getInt("id"));
		}
		return seller;
	}
	...
	public boolean create(Seller seller) throws SQLException {
		boolean flag = false;
		String query = "INSERT INTO sellers (name, email, password, phone, address, image, company, bussiness_id)"
				+ "VALUES (?,?,?,?,?,?)";
		stmt = con.prepareStatement(query);
		...
		stmt.setInt(8, seller.getBusiness());
	...
	public boolean update(Seller seller) throws SQLException {
		boolean flag = false;
		String query = "UPDATE sellers SET name=?, email=?, phone=?, address=?, image=?, company=?, business_id=? WHERE id=?";
		stmt = con.prepareStatement(query);
		stmt.setString(1, seller.getName());
		stmt.setString(2, seller.getEmail());
